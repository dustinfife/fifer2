...
compare.fits = function(formula, data, model1, model2=NULL, return.preds=F, silent=F, report.se=F,...){
	...
	if (model1.type == "lmerMod" | model1.type == "glmerMod"){
		...
		pred.mod1 = data.frame(prediction = predict(model1, pred.values, type="probs", re.form=NA), model= model1.type)		
	} else  {
	...
	if (model2.type == "lmerMod" | model2.type == "glmerMod"){
		...
		pred.mod2 = data.frame(prediction = predict(model2, pred.values, type="probs", re.form=NA), model= model2.type)		
	} else {
